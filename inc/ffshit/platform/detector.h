...
static constexpr uint32_t   SGOLD_FF_ADDRESS_MASK       = 0x0FFFFFFF;
...
class Detector {
    public:
        ...
        Detector(const RawData &data, Type platform);

        static Ptr build(const RawData &data) {
            return std::make_shared<Detector>(data);
        }

        static Ptr build(const RawData &data, Type platform) {
            return std::make_shared<Detector>(data, platform);
        }

        const Type          get_platform() const;
